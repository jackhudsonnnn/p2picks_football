...
export interface NbaSpreadTheWealthConfig {
  spread?: string | null;
...
export interface NbaSpreadTheWealthEvaluationResult {
  homeScore: number;
  awayScore: number;
...
export function normalizeSpread(config: NbaSpreadTheWealthConfig): number | null {
...
export function describeSpread(config: NbaSpreadTheWealthConfig): string | null {
...
export async function resolveTeams(
  config: NbaSpreadTheWealthConfig,
  league: League = 'NBA',
...
export async function evaluateSpreadTheWealth(
  config: NbaSpreadTheWealthConfig,
  spread: number,
): Promise<NbaSpreadTheWealthEvaluationResult> {
  const { homeTeam, awayTeam } = await resolveTeams(config, 'NBA');
...
function lookupTeam(teams: any[], id?: string | null, name?: string | null, homeAway?: 'home' | 'away'): any | null {
  ...
  if (normalizedId) {
    const byId = teams.find((team) => team?.teamId === normalizedId || team?.abbreviation === normalizedId);
    if (byId) return byId;
  }
  ...
  if (normalizedName) {
    const byName = teams.find(
      (team) =>
        String(team?.displayName ?? '').trim().toLowerCase() === normalizedName ||
