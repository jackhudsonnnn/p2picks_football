...
import { getHomeTeam, getAwayTeam, getPlayerStat } from '../../../../services/leagueData';
...
export async function prepareEitherOrConfig({ bet, config }: { bet: BetProposal; config: Record<string, unknown> }): Promise<Record<string, unknown>> {
  ...
  const allowedResolveAt = [...ALLOWED_RESOLVE_AT];
  const resolveAt = cfg.resolve_at ? String(cfg.resolve_at) : '';
  if (!resolveAt || !allowedResolveAt.includes(resolveAt as (typeof ALLOWED_RESOLVE_AT)[number])) {
    cfg.resolve_at = DEFAULT_RESOLVE_AT;
  } else {
    cfg.resolve_at = resolveAt as (typeof ALLOWED_RESOLVE_AT)[number];
  }

  cfg.progress_mode = normalizeProgressMode(cfg.progress_mode);

  const statKey = cfg.stat ? String(cfg.stat) : '';
  const category = NBA_STAT_KEY_TO_CATEGORY[statKey];
  ...
  try {
    ...
    return {
      ...
      bet_id: bet.bet_id,
      stat_label: NBA_STAT_KEY_LABELS[statKey] || cfg.stat_label || statKey,
