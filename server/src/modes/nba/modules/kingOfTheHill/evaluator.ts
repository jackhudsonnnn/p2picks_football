...
import { readPlayerStatValue, resolvePlayerKey, resolveStatKey as baseResolveStatKey } from '../../utils/statEvaluatorHelpers';
...
export function determineProgressOutcome(progress: ProgressRecord): ProgressOutcome {
  ...
  if (player1Reached && !player2Reached) return 'player1';
  if (player2Reached && !player1Reached) return 'player2';
  if (player1Reached && player2Reached) {
    const resolved = resolveSimultaneousFinish(progress);
    if (resolved === 'tie') return 'tie';
...
function updatePlayerProgress(
  ...
  if (current.reached) {
    return { ...current, baselineValue: baseline, lastValue: latestValue };
  }
  if (metric >= threshold) {
    return {
      ...current,
...
function resolveSimultaneousFinish(progress: ProgressRecord): 'player1' | 'player2' | 'tie' {
  ...
  if (Number.isFinite(ts1) && Number.isFinite(ts2) && ts1 !== ts2) return ts1 < ts2 ? 'player1' : 'player2';
  ...
  if (metric1 != null && metric2 != null && metric1 !== metric2) return metric1 > metric2 ? 'player1' : 'player2';
  const value1 = valueFromProgress(progress.player1);
  const value2 = valueFromProgress(progress.player2);
  if (value1 != null && value2 != null && value1 !== value2) return value1 > value2 ? 'player1' : 'player2';
  return 'tie';
}

function ensureBaselineValue(player: PlayerProgress | undefined, fallback: number): number {
  if (player && Number.isFinite(player.baselineValue)) return player.baselineValue;
...
function metricFromProgress(player: PlayerProgress, progressMode: 'starting_now' | 'cumulative'): number | null {
  if (typeof player.metricAtReach === 'number' && Number.isFinite(player.metricAtReach)) return player.metricAtReach;
  if (progressMode === 'starting_now') return computeDelta(player.lastValue, player.baselineValue);
  if (Number.isFinite(player.lastValue)) return player.lastValue;
  return null;
}

function valueFromProgress(player: PlayerProgress): number | null {
  if (typeof player.valueAtReach === 'number' && Number.isFinite(player.valueAtReach)) return player.valueAtReach;
  if (Number.isFinite(player.lastValue)) return player.lastValue;
  return null;
}
