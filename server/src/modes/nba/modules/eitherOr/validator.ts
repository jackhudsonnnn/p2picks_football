...
import { NBA_EITHER_OR_PLAYER_STAT_MAP, NBA_EITHER_OR_STORE_PREFIX, NBA_EITHER_OR_CHANNEL, NBA_EITHER_OR_MODE_KEY, NBA_EITHER_OR_LABEL, NBA_EITHER_OR_RESULT_EVENT, NBA_EITHER_OR_BASELINE_EVENT } from './constants';
import { evaluateEitherOr, EitherOrBaseline, EitherOrConfig, buildEitherOrBaseline } from './evaluator';

export class NbaEitherOrValidatorService extends BaseValidatorService<EitherOrConfig, EitherOrBaseline> {
  constructor() {
    super({
      league: 'NBA',
      modeKey: NBA_EITHER_OR_MODE_KEY,
      channelName: NBA_EITHER_OR_CHANNEL,
      storeKeyPrefix: NBA_EITHER_OR_STORE_PREFIX,
      modeLabel: NBA_EITHER_OR_LABEL,
      resultEvent: NBA_EITHER_OR_RESULT_EVENT,
      baselineEvent: NBA_EITHER_OR_BASELINE_EVENT,
    });
  }
...
export const eitherOrValidator = new NbaEitherOrValidatorService();
...
function resolveStatKey(config: EitherOrConfig | null | undefined): string | null {
  ...
  if (!statKey || !NBA_EITHER_OR_PLAYER_STAT_MAP[statKey]) return null;
  return statKey;
}
