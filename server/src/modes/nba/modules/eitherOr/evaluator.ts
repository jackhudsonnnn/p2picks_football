...
import { NBA_EITHER_OR_PLAYER_STAT_MAP } from './constants';
...
function resolveStatKey(config: EitherOrConfig | null | undefined): string | null {
  return baseResolveStatKey(config?.stat, NBA_EITHER_OR_PLAYER_STAT_MAP);
}

async function getPlayerValue(league: League, gameId: string, ref: PlayerRef, statKey: string): Promise<number> {
  const value = await readPlayerStatValue(league, gameId, ref, statKey, NBA_EITHER_OR_PLAYER_STAT_MAP);
  return value ?? 0;
}
