...
export const chooseTheirFateModule: ModeModule = {
  definition: {
    ...
    winningConditionTemplate:
      '`${(config.possession_team_name || config.possession_team_id || "Offense")}\'s drive result in the ${(config.home_team_name || config.home_team_id || "Home Team")} vs ${(config.away_team_name || config.away_team_id || "Away Team")} game`',
